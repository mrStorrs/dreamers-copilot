...
| **Nova** | Persona | Planning specialist. Mirrors `/dreamers-plan` discovery and full-spec planning. `/agents nova`. |
...
| `/dreamers-plan` | 3-phase planning (interactive Hash-out → Write → Review). Explores the codebase before asking discoverable questions, walks dependent design decisions with recommended answers, critiques the proposal before approval, writes full-spec plan file(s) + optional manifest, then verifies plan coverage against the proposal and user discussion before the review gate. Hard-stops at the review gate. |
...
/dreamers-full <task | plan paths | manifest.md>
  ├─ Phase 1   → /dreamers-plan   (Mode 1 only: discovery interview + full-spec planning; plan/manifest modes skip)
