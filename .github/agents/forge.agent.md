...
**Stage by explicit path only.** Never use `git add -A`, `git add --all`, `git add -a`, `git add .`, or any other "add everything" invocation — they silently capture unrelated edits from other agents' lanes or stray local files. Pass each path to `git add` directly (e.g. `git add src/auth/login.ts src/auth/session.ts`). See `~/.copilot/dreamers/refs/git-workflow.md` → Staging hygiene.
