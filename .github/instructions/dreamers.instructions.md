...
- **Quality gates are mandatory for PR-bearing code-change workflows.** Full-pipeline (Tier 2) work requires the `standard` review lane before PR creation: the orchestrator runs type-checks/tests inline, then Sentinel reviews and Probe audits coverage. Hone is trigger-based, not default; add it only for architecture/refactor risk or explicit full-review requests. Documented exceptions: (1) Tier 1 lightweight fixes (orchestrator implements inline → tests run inline → Sentinel review → close-out, no Probe/Hone); (2) maintenance/cleanup flows (e.g. `/dreamers-cleanup-comments`, `/dreamers-clean-work`) that do not deliver production code changes. No other exceptions — skipping Sentinel or Probe on a full-pipeline feature because "the work is simple" is not allowed.
