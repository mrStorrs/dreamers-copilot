...
**Subagent prompt rule:** when spawning Echo below, include the line "Do NOT call `manage_todo_list`. The orchestrator owns the todo." in Echo's prompt. Per `dreamers-kernel.md` § "Single-owner todo".
...
Pass in the prompt (per `dreamers-kernel.md` § "Subagent prompt — required content"):
