...
5. **User-testing-required check** — if the sub-plan's `User testing required: yes`**pause by calling the `request_info` tool**. The call must include every item required by `~/.copilot/dreamers/refs/sub-plan-loop.md` → "User testing pause rule" → "`request_info` content (mandatory)" — sub-plan ID + path, build distribution details, what changed, step-by-step test steps derived from the sub-plan's Acceptance Criteria and Probe's Given/When/Then cases, known limitations, and the approve / `Bug: <desc>` response format. Follow the resume rules in that ref: on approval proceed to step 6; on any bug, re-Sentinel → re-Probe → re-distribute → re-call `request_info` (do not commit until explicit approval).
6. **Bolt commits the sub-plan** — `task(agent_type: "bolt", mode: "sync")` — single commit covering all staged changes for this sub-plan. Conventional Commits, imperative mood, ≤72 chars subject. Reference the plan in commit body (`Plan: plan-{slug}-a`). One commit per sub-plan.
