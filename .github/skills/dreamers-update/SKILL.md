...
4. **Delivery ownership parity.** Keep planning, implementation, review execution, documentation, and PR details in their owning specialized skills. Keep Copilot `.github/skills/dreamers/SKILL.md` and Codex `skills/dreamers/SKILL.md` limited to orchestration, finding disposition, user testing, gates, and close-out; transfer semantics without copying specialist internals into either orchestrator.
...
Report in chat: Copilot files changed, Codex files changed, sync checks performed in each repo (refs / `dreamers` Phase 2 mirror / READMEs / catalog), validations run, halts or questions raised.
